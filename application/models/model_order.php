...
public function pembeli($pembeli,$id_produk){
	...
	public function update_stok($jumlah_beli,$id_produk){
				$this->db->query("update stok set stok=stok-$jumlah_beli where id_produk='$id_produk'");			
				 }
...
function tampil_invoice($id_pesan){ 
	 $this->db->select('pesanan.id_pesan,produk.nama as namaProduk,  detail_pesan.jml_pesan as jumlah, produk.harga as harga,bayar.total_bayar as totBayar'); 
  	 $this->db->from('bayar');
