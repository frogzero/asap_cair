...
function tampilSemuaPesanan(){
	...
	$this->db->select('bayar.id_bayar as kodeBayar,bayar.id_pesan as idPesan, pesanan.tgl_pesan as tanggalPesan, bayar.gambar_konfirmasi as download_gambar, pesanan.nama_penerima as namaPem, bayar.total_bayar as totalBayar, bayar.status, bayar.resi,bayar.id_pesan as idBayar,konsumen.nama as namaKom,bayar.tanggal_konfirmasi as tanggalBayar'); 
  	 $this->db->from('bayar');
 	 $this->db->join('pesanan', 'bayar.id_pesan = pesanan.id_pesan'); 
 	 $this->db->join('konsumen', 'pesanan.id_konsumen = konsumen.id_konsumen'); 
 	 $this->db->order_by('bayar.status','DESC');
...
function tampilProdukNota($id_pesan){ 
	 $this->db->select('pesanan.id_pesan,produk.nama as namaProduk,  detail_pesan.jml_pesan as jumlah, produk.harga as harga'); 
  	 $this->db->from('bayar');
 	 $this->db->join('pesanan', 'bayar.id_pesan = pesanan.id_pesan'); 
