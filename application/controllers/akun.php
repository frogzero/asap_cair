...
class Akun extends CI_Controller {
	...
	function member_login(){
		...
		$hasil1 = $this->model_login->login_pelanggan($nama,$password);
		if($hasil1==0){
			$nama = $this->input->post('nama');
			$password = $this->input->post('password');
			$hasil2 = $this->model_login->login_admin($nama,$password);
			if($hasil2==0){
		...
		public function ubah_passwortd()
	{
		$this->load->view('admin/index');
	...
	public function password_baru()
	{
		...
		$this->model_login->ubah_password($pswd,$id);
		echo '<script language="javascript">';
		echo 'alert("Password Sudah Di Ganti. silahkan Login Ulang")';
		echo '</script>';
		echo '<script type="text/javascript">';    
     	echo 'window.location.assign("'.site_url('akun/logout').'")'; 
      	echo '</script>';  
...
public function tampilProfil($id_konsumen=''){
	 $data['kategori'] = $this->model_kategori->tampilKategori();
		...
		$this->load->view('web_view/profil',$data);
